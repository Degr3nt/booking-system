...
function attachResourceNameValidation(input) {
  const update = () => {
    ...
    setButtonEnabled(createButton, allResourcesValid());
  };
...
function allResourcesValid() {
  return !(resourceNameInput.value.trim()==="") && isResourceNameValid(resourceNameInput.value) && !(resourceDescriptionInput.value.trim()==="") && isResourceDescriptionValid(resourceDescriptionInput.value);
}