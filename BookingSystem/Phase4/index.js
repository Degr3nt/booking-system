...
const resourceValidators = [
  ...
  body('resourceName')
    ...
    .matches(/^[a-zA-Z0-9äöåÄÖÅ ]+$/) //fixed error (missing regex)
    .escape()
  ...
  body('resourceDescription')
    ...
    .matches(/^[a-zA-Z0-9äöåÄÖÅ ]+$/) //fixed error (missing escape)
    .escape().withMessage('Resource description must be 10-50 characters, letters/numbers/spaces only'),
  ...
  body('resourcePrice')
    .exists({ checkFalsy: false }).withMessage('resourcePrice is required')
    .default(0)
  ...
  body('resourcePriceUnit')
    ...
    .trim()
    .isIn(['hour', 'day', 'week', "month"]) //fixed error
...
app.post('/api/resources', resourceValidators, async (req, res) => {
  ...
  //resourceAvailable = false; //fixed error (unwanted behavior)
  ...
  try {
    ...
    `;
    const params = [
      resourceName, //fixed error (hashing name)
      resourceDescription,
      Boolean(resourceAvailable),
      Number(resourcePrice), //fixed error (doubled price)
      resourcePriceUnit
    ];
